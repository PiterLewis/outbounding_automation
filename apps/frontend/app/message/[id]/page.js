...
export default function MessageEditor() {
  ...
  const [asunto, setAsunto] = useState(
    esNuevo ? "" : "No te lo pierdas! Comedy Night es manana"
  );
  const [cuerpo, setCuerpo] = useState(
    esNuevo
      ? ""
      : "Hola [Nombre], consigue tus entradas antes de que se agoten! Nos preparamos para una noche increible de comedia.\n\nNo dejes pasar esta oportunidad unica."
  );

  const tabsPanel = ["etiquetas", "chat", "manual"];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full md:w-72 lg:w-80 border-b md:border-b-0 md:border-r border-gray-200 bg-white overflow-auto">
            ...
            {tabsPanel.map((tab) => (
              <button
                ...
                className={`px-3 py-2 text-xs capitalize transition-colors ${
                  tabActiva === tab
                    ? "bg-gray-100 text-gray-900 font-medium rounded-t-md"
          ...
          <div className="p-4">
            {tabActiva === "etiquetas" && (
              <TagsPanel onGenerar={(texto) => setCuerpo(texto)} />
            )}
            {tabActiva === "chat" && (
              <ChatPanel onAplicar={(msg) => setCuerpo(msg)} />
            )}
            {tabActiva === "manual" && (
